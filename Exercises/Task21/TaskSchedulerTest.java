...
class Schedulers {
    ...
    public Schedulers(List<Task> tasks) {
        this.tasks = tasks;
    }

    public static <T extends Task>

    TaskScheduler<T> getOrdered() {
    ...
    TaskScheduler<T> getFiltered(int order) {
        // vashiot kod ovde (lambda expression)
        return tasks1 -> Arrays.stream(tasks1).filter(i -> i.getOrder() <= order).collect(Collectors.toList());
    }
}