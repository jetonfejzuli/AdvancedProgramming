...
class University {
    ...
    public List<String> getAllCourseNames() {
        //TODO
//        List<Course> courses = departments.stream()
...
//        return courses.stream().map(Course::getName).collect(Collectors.toList());
    ...
    public Map<String, List<String>> getDepartmentToCourseNames() {
        //TODO
        return departments.stream()
                .collect(Collectors.toMap(Department::getName, d -> d.getCourses().stream()
                        .map(Course::getName)
                        .collect(Collectors.toList())));
    }
    ...
    public Optional<Department> getMostPopularDepartment() {
        //TODO
        int sum = departments.stream()
                .flatMap(d -> d.getCourses().stream())
                .mapToInt(Course::getEnrolledStudents).sum();
        return departments.stream().max(Comparator.comparingInt(s -> sum));
    }
    ...
    public List<Course> getCoursesByDifficultyRange(int min, int max) {
        ...
        return departments.stream()
                .flatMap(d -> d.getCourses().stream())
                .filter(a -> a.getDifficulty() >= min && a.getDifficulty() <= max)
                .collect(Collectors.toList());
    }
    ...
    public List<String> getPopularCourseCodes(int minStudents) {
        ...
        return departments.stream()
                .flatMap(d -> d.getCourses().stream())
                .filter(d -> d.getEnrolledStudents() >= minStudents)
                .map(Course::getCode)
                .collect(Collectors.toList());
    }
    ...
    public Map<String, Integer> getTotalCreditsPerDepartment() {
        //TODO
        int totalCredits = departments.stream().flatMap(d -> d.getCourses().stream())
    ...
    public Map<String, Double> getAverageDifficultyPerDepartment() {
        //TODO
        double averageDifficulty = departments.stream()
                .flatMap(d -> d.getCourses().stream())
    ...
    public University mergeFourSmallestDepartments() {
        ...
        List<Department> smallestFour = departments.stream()
                .sorted(Comparator.comparingInt(d -> d.getCourses().stream()
                        .mapToInt(Course::getEnrolledStudents).sum()))
