...
class Scheduler<T> {
    ...
    public T next() {
        Date now = new Date();
        Date key = dates.ceilingKey(now);
        return dates.get(key);
    }

    public T last() {
        Date now = new Date();
        Date key = dates.floorKey(now);
        return dates.get(key);
    }

    public T getFirst() {
        return dates.get(dates.firstKey());
    }

    public T getLast() {
        return dates.get(dates.lastKey());
    }

    public ArrayList<T> getAll(Date begin, Date end) {
        return new ArrayList<>(dates.subMap(begin, true, end, true).values());
    }
}
